...
- Now lets create an `index.js` file inside the `pages` folder with the following code:
...
- As you might noticed we have a component that is shared by both `index.js` and `index.js` files, let's create that one now. Create a folder named `components` with a file named `header.js` in it and add the following code:
...
start now: run now in the other terminal
```
